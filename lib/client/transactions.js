...
var storeNeedInData = function(element, index, array){
	...
	data.push(obj);
	getNeeds(element.name, data, qty, date);
}

var getNeeds = function (name, data, parentQty, parentDate) {
	...
	if (typeof product !== "undefined") {
		...
		if (typeof needs !== "undefined") {
			...
			multipliedNeedsWithDate.forEach(storeNeedInData, parentProduct);
...
var getTransactionsData = function () {
	...
	for (var i = 0; i < nTransactions; i++) {
	  ...
	  obj.date = transactions[i].date;
		...
		data.push(obj);
		data = getNeeds(obj.name, data, parentQty, parentDate);
	}
